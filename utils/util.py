...
def print_size(net):
    ...
    paras = np.sum([np.prod(v.shape) for v in net.trainable_variables])
    print("{} parameters: {:.6f}M".format(
        net.__class__.__name__, paras/1e6), flush=True
    )
...
def calc_diffusion_step_embedding(diffusion_steps, diffusion_step_embed_dim_in):
    ...
    _embed = tf.math.exp(tf.arange(half_dim) * -_embed)
...
def training_loss(net, loss_fn, X, diffusion_hyperparams, only_generate_missing=1):
    ...
    diffusion_steps = tf.experimental.numpy.random.randint(1, T, size=(B, 1, 1))  # randomly sample diffusion steps from 1~T
...
def get_mask_rm(sample, k):
    ...
    # length_index = tf.tensor(range(mask.shape[0]))  # length of series indexes
    for channel in range(mask.shape[1]):
        perm = tf.range(mask.shape[0])
...
def get_mask_mnr(sample, k):
    ...
    mask = tf.ones(sample.shape)
    length_index = tf.range(mask.shape[0])
